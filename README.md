...
> Status: **`0.1.0` built** — the read-only memory core + Claude adapter (`ls`/`search`/`show`/`graph`/`stats`/`export`), **deterministic session profiles** (`sessions`/`session`), and the **memory↔session provenance bridge** (a memory shows the session it came from; a session lists the memories it produced; `stats`/`sessions` flag sessions that produced none). Next: `summarize`/`relevant` (`0.2.0`), curation (`0.3.0`), other agents (`0.4.0`). See [ROADMAP.md](ROADMAP.md) and [PLAN.md](PLAN.md).
...
| `summarize [--agent A --project P]` | "what have the agents learned?" digest | ✓ | — |
| `relevant <project>` | memories from *other* projects/agents that may apply here | ✓ | — |
...
Read-only commands are the default surface and work offline. LLM commands shell out to the `claude` CLI (reusing your existing auth — no API key). Mutating commands default to `--dry-run`, require `--apply`, never touch Codex's generated state or a locked sandbox.
...
- **`claude`** on `PATH` — only for `summarize` / `relevant`. Everything else works without it.
